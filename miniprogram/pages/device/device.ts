...
Page({
  ...
  setting1change()
  {
    let value = this.data.slider_value.toString(); //这个得是字符串， 这样方便json的打包和解包，
    setTimeout(() => {
      app.globalData.data2 = value; //同时更新到全局变量里， 用于富文本更新 
    }, 500);
    const jsonStr = JSON.stringify({ key: "data2", value: value });
    this.writeBLECharacteristicValue(jsonStr);
  },
  ...
  // 富文本更新
  updateRichText() {
    const data1 = app.globalData.data1 ;//|| 'NULL';
 ...
 createBLEConnection(e: { currentTarget: { dataset: any; }; }) {
  ...
  getBLEDeviceCharacteristics(deviceId: any, serviceId: string) {
    ...
    wx.onBLECharacteristicValueChange(characteristic => {
      ...
      // app.globalData.data1 = receivedData;
      // 解析数据并存储 这里从esp32收到的数据直接就是数值了， 所以直接存就好了(不过我又在esp32里加上了发送的前缀，也就是数据包头，所以还是先处理一下)
  ...
  writeBLECharacteristicValue(jsonStr: any) {
    ...
    wx.showToast({ title: "正在发送给装置", icon: "loading", duration: 5000});
    setTimeout(() => {
